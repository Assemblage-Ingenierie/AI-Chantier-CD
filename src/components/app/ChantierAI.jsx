...
export default function ChantierAI({ profile, session, onLogout, onProfileSaved }) {
  ...
  // anti-piégeage) : le tampon armé au chargement est ignoré, et le réarmement fait PENDANT
  // le popstate ne compte pas non plus (l'appui sur le bouton système n'est pas une
  // interaction sur la page). Diagnostic sur l'appareil de Thomas : 1er retour OK (consomme
  // l'unique entrée « légitime » du dernier tap), 2e retour → plus rien de non-sautable →
  // l'app se FERME (« mount » au milieu du journal). Correctif : chaque tap dépose une
  // RAFALE de 3 sentinelles (couvertes par la même activation utilisateur) → on survit à
  // 3 retours rapides d'affilée par tap, et chaque nouveau tap regarnit le tampon.
  // Throttle 1200ms → ≤ 75 pushState/30s, sous le plafond Chrome (~100/30s).
  useEffect(() => {
    let last = 0;
    const onDown = () => {
      const now = Date.now();
      ...
      last = now;
      try {
        for (let i = 0; i < 3; i++) history.pushState({ pwaSentinel: true }, '');
      } catch { /* throttle atteint — noop */ }
    };
    ...
    <div style={{ display:'flex',flexDirection:'column',height:'100dvh',width:'100%',fontFamily:"'Inter',system-ui,sans-serif",background:DA.grayXL }}>
      ...
      {!(isMobile && ouvert) && <div style={{ background:DA.white,borderBottom:`1px solid ${DA.border}`,padding:'8px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0 }}>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:8 }}>
          ...
          {/* Actualiser : icône visible UNIQUEMENT hors projet — dans un projet, le header
              noir (VisitesScreen/VueProjet) porte le sien → jamais deux boutons Actualiser. */}
          {!ouvert && (
            <button onClick={handleRefresh} disabled={refreshing} aria-label="Actualiser" title="Actualiser depuis le serveur"
              style={{ width:40,height:40,display:'flex',alignItems:'center',justifyContent:'center',background:'none',border:'none',color:DA.gray,cursor:refreshing?'default':'pointer',borderRadius:8 }}>
