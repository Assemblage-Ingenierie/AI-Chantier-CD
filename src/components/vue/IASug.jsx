...
export default function IASug({ content, commentaire, photos = [], onApply, onApplyTitle, onApplyUrgence }) {
  ...
  const ask = async () => {
    ...
    try {
      ...
      const prompt = hasCtx
        ? `Observation de chantier :\nTitre : "${titre}"\n${photo}\n${texte ? `Ce que l'auteur a déjà écrit : "${texte}"` : ''}\n\nProduis 3 à 5 apports techniques distincts qui ENRICHISSENT cette observation, sans répéter ce qui est déjà écrit. Choisis les angles les plus pertinents (n'invente rien) parmi : un point structurel ou un mécanisme de désordre non mentionné ; un élément à étayer, mesurer ou vérifier pour fiabiliser le constat ; une cause probable ou un risque d'évolution à anticiper ; une solution, une investigation ou un renforcement que l'auteur pourrait avoir négligé. Chaque apport : court (1 à 3 phrases), concret, directement intégrable. Qualité > quantité.\nFormat strict : "1. texte", "2. texte". Sans intro ni conclusion.`
        : `Observation de chantier : "${titre}"\nProduis 3 à 5 apports techniques courts et orientés structure pour cette observation : mécanisme probable du désordre, cause sous-jacente, risque d'évolution, mesures ou investigations à mener, principe de réparation ou de renforcement. Chaque apport : 1 à 3 phrases, concret, directement intégrable. Qualité > quantité.\nFormat strict : "1. texte", "2. texte". Sans intro ni conclusion.`;

      const d = await callAIProxy({
        feature: 'observation-suggestion',
        model: 'claude-sonnet-4-6',
        max_tokens: 2000,
        system: `Tu es un ingénieur structure senior (réhabilitation du bâtiment) qui relit l'observation d'un confrère sur le terrain. Ton rôle n'est PAS de décrire le désordre déjà constaté, mais d'APPORTER DE LA VALEUR : compléter l'analyse structurelle, signaler ce qui mérite d'être étayé ou vérifié, et proposer des pistes ou solutions que l'auteur n'a peut-être pas envisagées. Oriente toujours vers le comportement de la structure : mécanisme probable du désordre, cheminement des charges, cause sous-jacente, risque d'évolution, mesures ou investigations à mener, principe de réparation ou de renforcement. Style : concis, dense, factuel, expert, en français professionnel. Chaque commentaire est court (1 à 3 phrases) et autoporteur, directement intégrable dans un rapport de visite. Ne paraphrase pas l'auteur, ne répète pas le constat évident. Orthographe parfaite. N'utilise jamais le tiret médian (—) ni les tirets longs. Pas de listes ni de puces à l'intérieur d'un commentaire.`,
