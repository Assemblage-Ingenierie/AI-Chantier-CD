...
// onSelect(num)      → mode sélection unique (PlanLocModal)
// onSelectMany(nums) → mode multi-sélection (PlanLibraryModal)
export default function PdfPagePicker({ pdfData, onSelect, onSelectMany, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setPages(t);
          // En multi-sélection : tout coché par défaut
          if (multiMode) setSelected(new Set(t.map(p => p.num)));
          setLoading(false);
        }
      } catch (e) {
        if (!cancelled) {
  ...
  const toggleAll = () => {
    setSelected(selected.size === pages.length ? new Set() : new Set(pages.map(p => p.num)));
  };
      ...
      <div style={{ background:DA.black,padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0 }}>
        <div>
          <p style={{ color:'white',fontWeight:700,fontSize:14,margin:0 }}>
            {multiMode ? 'Sélectionner les pages à importer' : 'Choisir une page'}
          </p>
          {total > 0 && (
            <p style={{ color:DA.grayL,fontSize:11,margin:'2px 0 0' }}>
              {total} page{total>1?'s':''}
              {multiMode && selected.size > 0 && ` — ${selected.size} sélectionnée${selected.size>1?'s':''}`}
            </p>
          )}
        </div>
        <div style={{ display:'flex',gap:8,alignItems:'center' }}>
          {multiMode && !loading && !err && pages.length > 0 && (
            <>
              <button onClick={toggleAll}
                style={{ background:'rgba(255,255,255,0.12)',color:'white',border:'none',borderRadius:8,padding:'6px 12px',fontSize:11,fontWeight:600,cursor:'pointer',whiteSpace:'nowrap' }}>
                {selected.size === pages.length ? 'Désélectionner tout' : 'Tout sélectionner'}
              </button>
              <button onClick={() => selected.size > 0 && onSelectMany([...selected].sort((a,b)=>a-b))}
      ...
      <div style={{ flex:1,overflowY:'auto',padding:16 }}>
        ...
        {err && (
          <div style={{ background:'#FFF0F0',border:'1px solid #FCA5A5',borderRadius:10,padding:16,textAlign:'center' }}>
            ...
            <button onClick={onClose} style={{ fontSize:12,padding:'6px 16px',borderRadius:8,background:DA.red,color:'white',border:'none',cursor:'pointer',fontWeight:600 }}>Fermer</button>
          </div>
        )}
        {!loading && !err && (
          <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10 }}>
            {pages.map(pg => {
              ...
              return (
                <button key={pg.num}
                  onClick={() => multiMode ? togglePage(pg.num) : onSelect(pg.num)}
                  style={{ background:isSel?'rgba(227,5,19,0.15)':'#333',borderRadius:8,overflow:'hidden',border:`2px solid ${isSel?DA.red:'transparent'}`,cursor:'pointer',padding:0,position:'relative',transition:'all 0.1s' }}
                  onMouseEnter={e => { if (!multiMode) e.currentTarget.style.borderColor=DA.red; }}
                  onMouseLeave={e => { if (!multiMode) e.currentTarget.style.borderColor='transparent'; }}>
                  {multiMode && (
                    <div style={{ position:'absolute',top:6,right:6,width:20,height:20,borderRadius:4,background:isSel?DA.red:'rgba(255,255,255,0.2)',border:isSel?'none':'1.5px solid rgba(255,255,255,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1 }}>
                      {isSel && <span style={{ color:'white',fontSize:12,fontWeight:700,lineHeight:1 }}>✓</span>}
                    </div>
                  )}
                  <img src={pg.thumb} alt={`p${pg.num}`} style={{ width:'100%',height:'auto',display:'block' }}/>
                  <p style={{ textAlign:'center',padding:'6px 0',fontSize:11,color:isSel?'white':DA.grayL,margin:0,fontWeight:isSel?700:400 }}>Page {pg.num}</p>
                </button>
              );
            })}
          </div>
